# ✨ Magia de Emojis ✨

Una aplicación web sencilla y mágica diseñada para niños. Permite interactuar con el teclado y el ratón en un entorno seguro y visualmente estimulante.

## 🚀 Características

- **Estrellas Interactivas**: Un fondo de estrellas que parpadean y huyen del ratón (efecto de repulsión).
- **Explosión Dinámica**: Los emojis ahora salen disparados en todas direcciones y rotaciones.
- **Modo Pantalla Completa**: Un botón grande y fácil de usar para entrar en modo inmersivo.
- **Rendimiento Optimizado**: Protege tu navegador con un límite de 50 emojis activos, ideal para que los peques aporreen el teclado sin lag.
- **Protección Antitravesuras**: Bloquea las teclas de función (F1-F12, excepto F11 para salir) y las recargas accidentales.
- **Adaptativo y Estable**: Redimensionado de estrellas inteligente (debouncing) para mayor estabilidad.
- **Efectos de Sonido**: Sonidos de "pop" y accesibilidad (`aria-live`) integrada.

## 🛠️ Cómo ejecutarla (Rápido y Fácil) 🚀

### Opción 1: Directo en tu navegador
¡No necesitas descargar nada! Haz clic en el siguiente enlace para empezar a jugar:
👉 [**Ver Magia de Emojis en Vivo**](https://ctagadev.github.io/magia-de-emojis/)

### Opción 2: Descarga el ZIP oficial
Si prefieres tenerlo en tu ordenador para jugar sin internet:

1.  **Descarga el ZIP** desde la versión oficial: [**magia-emojis v1.2.2**](https://github.com/ctagadev/magia-de-emojis/releases/tag/v1.2.2)
2.  Descomprímelo en una carpeta de tu ordenador.
3.  Haz doble clic en `index.html`.
4.  ¡Pulsa el botón **"¡Empezar Fiesta! 🎉"** para entrar a pantalla completa y que empiece la magia! ✨

---

### Opción para Desarrolladores (Servidor local)
Si tienes Node.js instalado y quieres trastear con el código:

```bash
npm run dev
```

Esto abrirá la aplicación en `http://localhost:3000`.

## 🎹 Controles

- **Ratón**: Muévelo para que las estrellas huyan del puntero.
- **Teclado**: Pulsa cualquier tecla para que aparezcan emojis mágicos.
- **F11**: Entrar o salir de pantalla completa.

---
Idea original de un **padre creativo** y materializada por **Antigravity** utilizando **Vibe Coding** ✨. Hecho con ❤️ para que los más peques disfruten de la tecnología.
